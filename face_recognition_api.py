...
def load_image_file(filename, mode='RGB'):
    ...
    # Convert image to RGB or L mode if necessary
    if mode == 'RGB' and img.mode != 'RGB':
        img = img.convert('RGB')
    elif mode == 'L' and img.mode != 'L':
        img = img.convert('L')
    ...
    if img.shape[0] > 800:
        ...
        p = int(img.shape[1] * w)
        img = np.array(Image.fromarray(img).resize((p, baseheight), Image.BILINEAR))
    elif img.shape[1] > 800:
        baseheight = 500
        w = (baseheight / img.shape[1])
        p = int(img.shape[0] * w)
        img = np.array(Image.fromarray(img).resize((baseheight, p), Image.BILINEAR))
